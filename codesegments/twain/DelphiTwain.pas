...
{This is my newest contribution for Delphi comunity, a powerful }
...
begin
  ...
  case Value of
    {Successful, copy handle and return a success value}
  ...
  if Result = TWRC_SUCCESS then
    repeat
      ...
      Result := Owner.TwainProc(AppInfo, @Structure, DG_IMAGE,
        DAT_IMAGEMEMXFER, MSG_GET, @Xfer); // debug: DAT_IMAGEMEMXFER replaced by DAT_IMAGENATIVEXFER
      {Test the result}
      ...
      begin
        ...
        begin
          // hw: added some error handling and messages in case errors are left unhandled
          //
...
begin
  ...
  Handle := GlobalAlloc(GHND, StructSize); // allocate global memory
  Dib := GlobalLock(Handle); // get a pointer to the global memory object
  ...
  // calculate Image Data Pointer
  Cardinal(Data) := Cardinal(Dib) + // offset vector
                    Dib^.bmiHeader.biSize + // header size
                    (Dib^.bmiHeader.biClrUsed * sizeof(RGBQUAD)); // palette size
...
begin
  ...
  //Enabled := False;
  // hw:
  // commented out, disabling the source prematurely aborts
  // capture capability while user interface is still running.
  // Once the UI is closed by the user, the OnSourceDisable event
  // is triggered in DisableSource while handling VirtualWindow
  // messages.
  // acquiring data with ShowUI set to false triggers the event, too.
  // disabling is not needed at this point at all.
end;
