   ...
   begin
      ...
      begin
         ...
         repeat // swap and iterate through all elements from both sides until indices meet/cross
            if LeftIdx < rangeend-1 then
              repeat inc(leftidx)  until (Items[LeftIdx].ID  >= criticalval) {or (LeftIdx > rangeend - 2)}; // iterate inwards from the left
            if rightidx > 0 then
              repeat dec(rightidx) until (Items[RightIdx].ID <= criticalval) or (RightIdx = 0); // iterate inwards from the right
