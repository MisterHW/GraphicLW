...
public
  ...
  property ValidConnection     : Boolean read ConnectionIsValid;
...
begin
  ...
  InAction  := (HasObject or AutoSelectNext) and (ssLeft in Shift); // either a polygon is assigned or it can be created when needed
...
begin
  ...
  begin // add point
    FirstPointRefIndex := Poly.InsertPointReference(pt.UID,RemoteGLM.ResolvePointGroupID(pt), FirstPointRefIndex+1);
    RemoteGLM.ClientPaint(self);
  end
  else
