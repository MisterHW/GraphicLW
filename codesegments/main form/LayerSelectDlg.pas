...
var
  ...
  begin
    ...
    if not multiselect then // hier bräuchte es für multiselect noch einen Vorwahlrahmen,
                            // mit dem man dan mit +/- hinzufügen / entfernen kann
    begin
...
begin
  ...
  case (key and $FF) of                       // maybe move ENTER and ESC to KeyUp?
     VK_RETURN : if okbutton.Enabled then okbuttonClick(self); // ENTER
     VK_ESCAPE : abortbuttonClick(self);      // ESC
     VK_LEFT   : ChangeSingleSelection(-1,0); // <
     VK_UP     : ChangeSingleSelection(0,-1); // ^
     VK_RIGHT  : ChangeSingleSelection(+1,0); // >
     VK_DOWN   : ChangeSingleSelection(0,+1); // v
   end;
