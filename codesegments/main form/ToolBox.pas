...
var
  ...
  function TabSheetByTag(Tag: longint): TTabSheet;
  var i : integer;
  begin
    result := nil;
    for i := 0 to PageControl1.PageCount - 1 do
    begin
      if PageControl1.Pages[i].Tag = Tag then
      begin
...
begin
  ...
  begin
    ...
    begin
      ThisPage := TabSheetByTag((Sender as TSpeedButton).Tag);

      if assigned(ThisPage) then
      begin

        ThisPage.TabVisible := not ThisPage.TabVisible; // activate/deactivate page

        if ThisPage.TabVisible then
           PageControl1.ActivePageIndex := ThisPage.PageIndex; // not sure if this is too confusing  (change focus may distract from train of thought)...
